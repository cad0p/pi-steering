...
| "add a one-off check to a single rule" | Name it with `definePredicate` and wire through `requires:` — never inline `condition:` (`condition:` is forbidden in examples, CI-pinned). `requires: (ctx) => mySignal(null, ctx)` reads linearly with no registry or `declare global` augmentation; the const graduates to a plugin registry untouched when the pack grows. See `examples/force-push-strict/steering.ts`. |
