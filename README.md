...
**Fail-closed.** Exemption clauses evaluate with an "allow"-default projection — the OPPOSITE default from rule `when:` clauses. A predicate that can't resolve (walker-unknown cwd, a throwing handler, an unregistered predicate key) counts as "does not match", so the guard still fires. A carve-out can never silently open a guard: unknown state means the rule keeps blocking. (Authors who want unknown-to-exempt can write an explicit `onUnknown: "block"` inside the exemption clause.)
