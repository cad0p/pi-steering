...
| "require Y before X" | Observer that `appendEntry`s a marker, plus a rule whose `when.missing` gates on it (`{ event, in: "agent_loop" }`). Prefer this to hand-rolled `findEntries` + `agentLoopIndex` comparisons — same semantics, less code. |
